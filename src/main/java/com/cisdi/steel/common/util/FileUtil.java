...
public class FileUtil {
    ...
    public static void touch(File file) {
        ...
        if (!file.exists()) {
            ...
            System.err.println("Create quartz new file:" + file.getName());
            try {
     ...
     * @since 1.0
     */
    public static List<File> allFilesChild(File file) {
        ArrayList<File> res = new ArrayList<>();
        listAllfiles(file, res);
        return res;
    }

    public static void listAllfiles(File file, ArrayList<File> res) {
        File[] arr = file.listFiles();
        if (null != arr && arr.length > 0) {
            for (int i = 0; i < arr.length; i++) {
                if (arr[i].isDirectory()) {
                    listAllfiles(arr[i], res);
                } else {
                    res.add(arr[i]);
                }
            }
