...
public class ExportWordUtil {
    ...
    public static void main(String[] args) {
        Date date = new Date();
        try {
            XWPFDocument doc = new ExportWordUtil().export(null);
            String fileName = "五烧1" + DateUtil.getFormatDateTime(date, "yyyyMMdd") + "每日操业会 - 设计版v1.docx";
    ...
    public XWPFDocument export(byte[] base64Info1) {
        try {
            MyXWPFDocument xdoc = new MyXWPFDocument();
            // 创建页眉
            createCtp(xdoc);
     ...
     * @param imageByte
     */
    private void createPic(MyXWPFDocument xdoc, XWPFParagraph paragraph, byte[] imageByte) {
        try {
     ...
     * 创建标题
     */
    private void createTitle(MyXWPFDocument document) {
        // 标题
        XWPFParagraph titleParagraph = document.createParagraph();
        titleParagraph.setAlignment(ParagraphAlignment.CENTER);
        XWPFRun titleFun = titleParagraph.createRun();
        titleFun.setBold(true);
        titleFun.setFontFamily("华文仿宋");
        titleFun.setText("未来科技平台统计分析报告"); // 活动名称
        titleFun.setFontSize(18);
        titleFun.setColor("333333");
        titleFun.setBold(true);
    }
     ...
     * 生成页眉
     */
    public void createCtp(MyXWPFDocument document) {
     ...
     * @param xdoc
     */
    public void createBaseInfoTable(XWPFTable table, MyXWPFDocument xdoc, String dateStr, String unitName,
...
//		tblWidth.setType(STTblWidth.AUTO); // STTblWidth.AUTO 自动长度
     ...
     * @param titleText
     */
    public static void createTitle(MyXWPFDocument document, String titleText) {
        XWPFParagraph titleParagraph = document.createParagraph();
        titleParagraph.setAlignment(ParagraphAlignment.CENTER);
        XWPFRun titleFun = titleParagraph.createRun();
        titleFun.setBold(true);
        titleFun.setFontFamily("华文仿宋");
        titleFun.setText(titleText); // 活动名称
        titleFun.setFontSize(18);
        titleFun.setColor("333333");
        titleFun.setBold(true);
    }
     ...
     * @param titleText
     * @param wordTitleConfigDTO
     * @return
     */
    public static XWPFRun createTitle(XWPFParagraph titleParagraph, String titleText, WordTitleConfigDTO wordTitleConfigDTO) {
        titleParagraph.setAlignment(wordTitleConfigDTO.getParagraphAlignment());
        XWPFRun titleFun = titleParagraph.createRun();
        titleFun.setBold(wordTitleConfigDTO.getIsBold());
        titleFun.setFontFamily(wordTitleConfigDTO.getFontFamily());
        titleFun.setText(titleText);
        titleFun.setFontSize(wordTitleConfigDTO.getFontSize());
        titleFun.setColor(wordTitleConfigDTO.getColor());
        return titleFun;
    }
     ...
     * @return
     */
    public static XWPFRun createParagraph(XWPFDocument document, String titleText, WordTitleConfigDTO wordTitleConfigDTO) {
        XWPFParagraph titleParagraph = document.createParagraph();
        titleParagraph.setAlignment(wordTitleConfigDTO.getParagraphAlignment());
        XWPFRun titleFun = titleParagraph.createRun();
        titleFun.setBold(wordTitleConfigDTO.getIsBold());
     ...
     * @param xdoc
     */
    public void createDataReportTable(XWPFTable table, MyXWPFDocument xdoc, byte[] base64Info1) {
        String bgColor = "111111";
        CTTbl ttbl = table.getCTTbl();
        CTTblPr tblPr = ttbl.getTblPr() == null ? ttbl.addNewTblPr() : ttbl.getTblPr();
        CTTblWidth tblWidth = tblPr.isSetTblW() ? tblPr.getTblW() : tblPr.addNewTblW();
        ...
        setCellText(xdoc, getCellHight(table, 3, 0, 1200),
                "1:今速冻耳朵进我家我的叫我耳机都奇偶的价位。\n2：覅五黑喝完酒佛问服了你我发你我诶佛为IE金佛我解耦我金额为奇偶偶皮。\n\t\t\t2019-4-4 15:48:53\n\n" +
                        "1:今速冻耳朵进我家我的叫我耳机都奇偶的价位。\n2：覅五黑喝完酒佛问服了你我发你我诶佛为IE金佛我解耦我金额为奇偶偶皮。\n\t\t\t2019-4-4 15:48:53"
                ,
                bgColor, 4300, 14, "仿宋", ParagraphAlignment.LEFT, false);
     ...
     * 创建图片
     */
    private void setCellPic(MyXWPFDocument xdoc, XWPFTableCell cell, byte[] imageByte) {
        createPic(xdoc, cell.addParagraph(), imageByte);
    }

    private void setCellText(MyXWPFDocument xDocument, XWPFTableCell cell, String text, String bgcolor, int width,
    ...
    // 设置表格间的空行
    public void setEmptyRow(MyXWPFDocument xdoc, XWPFRun r1) {
