...
@Slf4j
public abstract class DrtAbstractExecute {
    ...
    public abstract IDrtWriter getDrtWriter();
    ...
    public void execute(DrtJobExecuteInfo drtJobExecuteInfo) {
        ...
        try {
            ...
            // 4、5填充数据
            Object fileObject = getDrtWriter().drtWriter(drtWriterDTO);
            this.createFile(fileObject, excelPathInfo);
            // 7、插入索引
     ...
     * @param excelPathInfo
     */
    public void createFile(Object fileObject, ExcelPathInfo excelPathInfo) throws IOException {
        FileOutputStream fos = new FileOutputStream(excelPathInfo.getSaveFilePath());
        // 隐藏 下划线的sheet  强制计算
        if (fileObject instanceof Workbook) {
            Workbook workbook = (Workbook) fileObject;
            ...
            workbook.write(fos);
        } else if (fileObject instanceof XWPFDocument) {
            XWPFDocument document = (XWPFDocument) fileObject;
            document.write(fos);
        }
        fos.close();
    }
